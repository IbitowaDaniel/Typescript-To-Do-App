...
export const TodoItem = (props: { todo: Todo }) => {
  ...
  return (
    <motion.div
      ...
      key={todo.id}
      className={cn(
        'p-5 rounded-xl bg-zinc-900 overflow-x-hidden bg-opacity-50',
        todo.status === 'completed' && 'text-zinc-500',
      )}
    >
      {editingTodoId === todo.id ? (
        <motion.div layout className="flex gap-2">
          ...
          />
          <button
            className="px-5 py-3 text-sm text-white font-normal bg-green-600 active:scale-95 rounded-lg"
